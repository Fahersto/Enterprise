...
/* Assertions.h
	...
	are stripped out of Release builds completely, while EP_VERIFYs evaluate to just
	the test expression in Release builds.
...
/// @endcond
	...
	/// Assert that an expression is true.  The expression is stripped out of Release builds.
	#define EP_ASSERT(expression)										EP_ASSERT_IMPL(expression, "")
	/// Assert that an expression is true, and log an error message if it's not.  The expression is stripped out of Release builds.
	#define EP_ASSERTF(expression, message)								EP_ASSERT_IMPL(expression, message)
	/// Assert that an expression is true.  Evaluates to just the expression in Release builds.
	#define EP_VERIFY(expression)										EP_ASSERT_IMPL(expression, "")
	/// Assert that an expression is true, and log an error message if it's not.  Evaluates to just the expression in Release builds.
	#define EP_VERIFYF(expression, message)								EP_ASSERT_IMPL(expression, message)
	/// Assert that an expression is equal to something.  Evaluates to just the first expression in Release builds.
	#define EP_VERIFY_EQ(expression, compareexpr)						EP_ASSERT_IMPL(expression == compareexpr, "")
	/// Assert that an expression is equal to something, and log an error message if it's not.  Evaluates to just the first expression in Release builds.
	#define EP_VERIFYF_EQ(expression, compareexpr, message)				EP_ASSERT_IMPL(expression == compareexpr, message)
	/// Assert that an expression is not equal to something.  Evaluates to just the first expression in Release builds.
	#define EP_VERIFY_NEQ(expression, compareexpr)						EP_ASSERT_IMPL(expression != compareexpr, "")
	/// Assert that an expression is not equal to something, and log an error message if it's not.  Evaluates to just the first expression in Release builds.
	...
	/// Execute code in assertion scenarios.  The expression is stripped out of Release builds.
	/// @warning Do not call code with side-effects in this code block, as it is stripped from Release builds.
	...
	/// Assert that a code path is only executed once.
	#define EP_ASSERT_NOREENTRY()
	/// Execute code in assertion scenarios.  The code is stripped out of Release builds.
	...
	/// Assert that an expression is true.  The expression is stripped out of Dev and Release builds.
	#define EP_ASSERT_SLOW(expression)									EP_ASSERT_IMPL(expression, "")
	/// Assert that an expression is true, and log an error message if it's not.  The expression is stripped out of Dev and Release builds.
	#define EP_ASSERTF_SLOW(expression, message)						EP_ASSERT_IMPL(expression, message)
	/// Assert that an expression is true.  Evaluates to just the expression in Dev and Release builds.
	#define EP_VERIFY_SLOW(expression)									EP_ASSERT_IMPL(expression, "")
	/// Assert that an expression is true, and log an error message if it's not.  Evaluates to just the expression in Dev and Release builds.
	#define EP_VERIFYF_SLOW(expression, message)						EP_ASSERT_IMPL(expression, message)
	/// Assert that an expression is equal to something.  Evaluates to just the first expression in Dev and Release builds.
	#define EP_VERIFY_EQ_SLOW(expression, compareexpr)					EP_ASSERT_IMPL(expression == compareexpr, "")
	/// Assert that an expression is equal to something, and log an error message if it's not.  Evaluates to just the first expression in Dev and Release builds.
	#define EP_VERIFYF_EQ_SLOW(expression, compareexpr, message)		EP_ASSERT_IMPL(expression == compareexpr, message)
	/// Assert that an expression is not equal to something.  Evaluates to just the first expression in Dev and Release builds.
	#define EP_VERIFY_NEQ_SLOW(expression, compareexpr)					EP_ASSERT_IMPL(expression != compareexpr, "")
	/// Assert that an expression is not equal to something, and log an error message if it's not.  Evaluates to just the first expression in Dev and Release builds.
	#define EP_VERIFYF_NEQ_SLOW(expression, compareexpr, message)		EP_ASSERT_IMPL(expression != compareexpr, message)
	/// Execute code in assertion scenarios.  The expression is stripped out of Dev and Release builds.
	/// @warning Do not call code with side-effects in this code block, as it is stripped from Release builds.
	...
	/// Execute code in assertion scenarios.  The code is stripped out of Dev and Release builds.
	/// @warning Do not call code with side-effects in this code block.
