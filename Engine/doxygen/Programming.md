...
* **Release**: Compiler optimizations are enabled, all assertions are disabled, and debug symbols are off.  Use this configuration to compile the shipping version of your game.
...
Predefined macros are available for conditionalizing code to run on only specific builds.  Use `#ifdef` or `#if defined()` to watch for the `EP_CONFIG_DEBUG`, `EP_CONFIG_DEV`, and `EP_CONFIG_RELEASE` defines.
...
For vector and matrix calculations, %Enterprise utilizes the [GLM math library](https://github.com/g-truc/glm).  It is integrated throughout the engine, and several %Enterprise API calls take GLM types as parameters.  Developers are encouraged to utilize GLM in their game projects.
