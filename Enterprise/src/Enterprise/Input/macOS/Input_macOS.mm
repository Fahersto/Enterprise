...
void Input::PlatformInit()
{
	...
	Events::SubscribeToType(EventTypes::macOS_keyEvent, &HandlePlatformEvents);
	Events::SubscribeToType(EventTypes::macOS_flagsChanged, &HandlePlatformEvents);
}

bool Input::HandlePlatformEvents(Events::Event& e)
{
	if (e.Type() == EventTypes::macOS_keyEvent)
	{
	...
	else if (e.Type() == EventTypes::macOS_flagsChanged)
	{
